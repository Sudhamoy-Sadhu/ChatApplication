    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isLogin) {
                ...
                try {
                    // 🔹 SIGNUP API CALL
                    const response = await axios.post('http://localhost:8080/signUp/register', {
                    ...
                    if (response.status === 200 || response.status === 201) {
                        const successMessage = response.data.message || 'Signup successful! Please login now.';
                    ...
                    if (error.response) {
                        ...
                        toast.error('⚠️ Something went wrong during signup.', { autoClose: 2500 });
                    }
                }
            }
        } catch (error) {
            ...
            if (error.response) {
                const errorMessage = error.response.data?.message || error.response.data || 'Invalid credentials';
                toast.error(`${errorMessage}`, { autoClose: 2500 });
            } else if (error.request) {
                // No response (server might be down)
                toast.error('⚠️ Server not responding. Please try again later.', { autoClose: 2500 });
            } else {
                // Unexpected error
